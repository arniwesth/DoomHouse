# DOOMHouse: The Architecture of Impossibility

"SQL is for data analysis."

I must have heard that phrase a thousand times. We use SQL to query tables, aggregate numbers, and maybe—if we're feeling adventurous—generate some complex reports. It's the reliable, boring workhorse of the backend.
...
I’m not talking about storing high scores. I’m talking about real-time, 3D raycasting, texture mapping, and lighting—all calculated inside a single SQL query.

Welcome to **DOOMHouse**.

## The Architecture of Impossibility

The concept sounds absurd on paper: a 3D game engine running entirely within ClickHouse. But the execution is where the real madness lies.

To make this run in real-time, we couldn't just run one big query. The computational load of calculating 307,200 pixels per frame—each requiring ray intersection, texture lookup, and shading—was too immense for a standard approach.

The solution was to split the workload into **four parallel pipelines**, a novel optimization technique for a database environment:

1.  **Raycasting**: Calculating the geometry and wall intersections.
2.  **Texture Mapping**: Looking up pixel data from dictionaries.
...
All of this happens *inside* the database. The Python client is effectively just a dumb terminal; it sends input and displays the resulting array of pixels. The database has become the GPU.
...
Instead, this project was constructed through an advanced **Agentic Workflow**. It wasn't just "asking AI to write code." It was a coordinated effort using multiple specialized models, treating them like a team of senior engineers:
...
Then I took the result and said, *"Optimize the previously optimized code."*
...
## Visual Debugging & Feedback Loops
...
So, we created a unique visual feedback loop. When the engine rendered a frame with a glitch—say, a wall that looked like it was melting or a floor that vanished—I took a screenshot.
