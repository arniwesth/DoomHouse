/*
   ========================================================================================
   DOOMHOUSE RENDER ENGINE: 3D Raycasting in Pure SQL (Split Pipeline)
   ========================================================================================
*/

-- =========================================================
-- VIEW 1: TOP HALF (Rows 0-239)
-- =========================================================
CREATE MATERIALIZED VIEW doomhouse.render_materialized_top
TO doomhouse.rendered_frame_top
AS
WITH 
    640 AS W,
    480 AS H,
...
(
    ...
    (
        ...
        CROSS JOIN (
            SELECT 
                number as y,
                toInt32(H - 1 - number) as dist_lookup_idx,
...
-- =========================================================
-- VIEW 2: BOTTOM HALF (Rows 240-479)
-- =========================================================
CREATE MATERIALIZED VIEW doomhouse.render_materialized_bottom
TO doomhouse.rendered_frame_bottom
AS
WITH 
    640 AS W,
    480 AS H,
...
(
    ...
    (
        ...
        CROSS JOIN (
            SELECT 
                number + H_HALF as y,
                toInt32(number + H_HALF) as dist_lookup_idx,
                dictGet('doomhouse.dict_floor_dist', 'dist', toUInt32(dist_lookup_idx + 1)) as floor_dist
            FROM numbers(H_HALF)
        ) AS v_lines
    ) AS sub
