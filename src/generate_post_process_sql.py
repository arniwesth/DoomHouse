...
-- VIEW {i}: STRIP {i}
-- =========================================================
CREATE MATERIALIZED VIEW doomhouse.post_process_materialized_{i}
TO doomhouse.rendered_frame_post_processed_{i}
AS
WITH
    640 AS w,
    image_data AS src,
...
FROM doomhouse.rendered_frame_{i};
"""

with open("src/SQL/post_process_view.sql", "w") as f:
    f.write("/* DOOMHouse 8-Way Split Post-Process Views */\n")
    for i in range(1, 9):
